...
firebase.auth().onAuthStateChanged((user) => {
  if (user) {
    ...
    cdn_listenForSnapshots_messages()
...
function cdn_listenForSnapshots_messages() {
  const user = auth.currentUser;
 
  if (user) {
    const itemsCollection = db.collection("messages").orderBy("createdAt", "desc"); // Replace with your collection name
